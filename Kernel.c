...
----------------------------- HoareCondition  ---------------------------------
  ...
  behavior HoareCondition
    ...
    --         It unlocks it, and goes to sleep waiting for a signal on
    --         this condition.  When the signal is received, this method
    --         re-awakens and returns i.e resumes execution.
    --    Signal(mutex)
    --         If there are any threads waiting on this condition, this
    --         method will wake up the oldest and schedule it to run, 
    --	       by giving the lock.
    --         The current thread that was running previously tries to acquire the mutex lock, 
    -- 	       but does not have heldby so goes to sleep and the thread that was on waiting 
    --	       resumes from where the cureent thread was stopped	
    --    Broadcast(mutex)
    --         This method is like Signal() except that it wakes up all
    --         threads waiting on this condition, not just the next one.
    --    Init()
    --         Each condition must be initialized.
      ...
      method Wait (mutex: ptr to HoareMutex)
          ...
          waitingThreads.AddToEnd (currentThread)
          currentThread.Sleep ()
          --mutex.Lock ()									--the thread in the waiting list has to resume the excution, so no lock needed	
          oldIntStat = SetInterruptsTo (oldIntStat)
        endMethod
  ...
  ---------- HoareCondition . Signal  ----------
	  ...
	  --  A: ptr to Thread
          ...
          if t
	   	mutex.GiveLock(t)								--Assigns heldby to the thread removed from the waiting list
            	t.status = READY
            	readyList.AddToEnd (t)
		mutex.Lock()									--the current thread tries to acquire the mutex lock, but does not have heldby so goes to sleep	 
          endIf
...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
	...
	aThreadBecameFree= new Condition						--creating a condition for thread Manager
	threadManagerLock= new Mutex							--creating a lock for thread Manager class
	...
	for j= 0 to MAX_NUMBER_OF_PROCESSES-1
		...
		freeList.AddToEnd(&threadTable[j])					--adding the thread to the freeList that now has status unused
	endFor
	...
	threadManagerLock.Lock()
        ...
        if (freeList.IsEmpty())									--check if the freeList is empty 
             while( ! threadInUse )		
                threadInUse = freeList.Remove()	
                if (! threadInUse )
                  aThreadBecameFree.Wait(&threadManagerLock)					--if the list is empty it makes the thread requesting to wait
               endIf
              endWhile
            else
              threadInUse = freeList.Remove()							-- if free list is not empty, then removes a thread
            endIf
  
          threadInUse.status = JUST_CREATED							--makes its status as Just_created
  
          threadManagerLock.Unlock()

          return threadInUse									--returns this new thread
	...
	threadManagerLock.Lock()
	  ...
	  th.status=UNUSED								-- the status of the thread passed is changed to unused
	  freeList.AddToEnd(th)							-- the  thread is added back of the waiting list again
          aThreadBecameFree.Signal(&threadManagerLock)						-- the requesting threads are now signalled, when the threads are now available
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
	...
	-- GetANewProcess () 
      ...
      var i:int=0
	processManagerLock=new Mutex							--creates a lock for process control block
	processManagerLock.Init()
	aProcessBecameFree=new Condition						-- creates a condition class 
	...
	for i=0 to MAX_NUMBER_OF_PROCESSES-1
		processTable[i].Init()							-- initializes a process table that holds the processes
		processTable[i].status=FREE						-- make all the status of the processes as free
		freeList.AddToEnd(&processTable[i])					-- add these processes to freeList
	endFor
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
          var
        processInUse:ptr to  ProcessControlBlock					--declares a object of process control block
	
        processManagerLock.Lock()

          if (freeList.IsEmpty())							-- checks if the free list is empty
             while( ! processInUse )
                processInUse = freeList.Remove()					-- if free list is empty then wait on the condition till a processcontrol block is available
                if (! processInUse )
                  aProcessBecameFree.Wait(&processManagerLock)
               endIf
              endWhile
            else
             processInUse = freeList.Remove()						-- if the free list not empty, then remove a process control block from free list
            endIf
	  
	nextPid= nextPid + 1								-- increments a process counter
	processInUse.pid=nextPid							-- assigns a id to a process
        processInUse.status = ACTIVE							-- make the status active of each process	

          processManagerLock.Unlock()

          return processInUse								-- returns the assigned process control block
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        processManagerLock.Lock()
		p.pid= -1						        -- assigns a default pid to the process 
        	p.status=FREE						-- sets the status as free
        	freeList.AddToEnd(p)					-- Add the process Control Block to the freeList
        aProcessBecameFree.Signal(&processManagerLock)					-- Signals whens process control block is available to requesting threads
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
	...
	-- ReturnAllFrames (aPageTable: ptr to AddrSpace)
	  ...
	  numWaitingGroups= 0 									-- initialized a variable that holds the waiting threads
          frameManagerLock = new Mutex
          frameManagerLock.Init ()
          newFramesAvailable= new Condition							--creates a condition for frames available, to the thread waiting at front
          newFramesAvailable.Init ()
	  numGroupsWaiting= new Condition							--creates a condition for waiting thread groups 
          numGroupsWaiting.Init ()
	...
	frameManagerLock.Lock()
		...
		if numWaitingGroups> 1 							--if count of waiting group is greater than one, makes the restof group to wait
			numGroupsWaiting.Wait(&frameManagerLock)                   
		endIf
		while(numberFreeFrames< numFramesNeeded )				-- if the frames are less than needed, then the thread requesting is made to wait
			newFramesAvailable.Wait( &frameManagerLock)
		endWhile
		for i=0 to numFramesNeeded-1						--else assigns the frame
			f=framesInUse.FindZeroAndSet()
			frameAddr= PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (f * PAGE_SIZE)
			aPageTable.SetFrameAddr(i,frameAddr)
		endFor
		
		numberFreeFrames=numberFreeFrames-numFramesNeeded			--updates the frame numbers 
		numWaitingGroups = numWaitingGroups-1					--updates the waiting group number  
		numGroupsWaiting.Broadcast(&frameManagerLock) 				-- broadcast when one thread acquires frame to the other thread
	...
	for i=0 to numFramesToReturn-1								-- frames not in Used added back to the free list
		frameAddr=aPageTable.ExtractFrameAddr(i)
		frameNumber= (frameAddr - PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME ) / PAGE_SIZE	--allocated address
		framesInUse.ClearBit(frameNumber)						--set bit as clear
		numberFreeFrames=numberFreeFrames+1						--updates number of frames
	endFor
	newFramesAvailable.Broadcast(&frameManagerLock)						--broadcast when frames available
	aPageTable.numberOfPages= 0								-- set page table to default
	frameManagerLock.Unlock()
        endMethod 
 ...
 ----------  ThreadManager . Print  ----------
  ...
  ----------  ThreadManager . GetANewThread  ----------
      ...
      var
        ...
        if (freeList.IsEmpty())								--Hoare semantic  to test condition
